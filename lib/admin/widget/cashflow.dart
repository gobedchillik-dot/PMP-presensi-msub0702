...
Future<void> generateCashflowPdf({
  ...
  required double totalUnpaidSalary,
  required double totalOtherExpenses,
  required double netProfit,
  required List<Map<String, dynamic>> weeklySummary,
  required List<Map<String, dynamic>> cashflowList,
}) async {
  ...
  // Helper tanggal untuk Timestamp / DateTime / String
  ...
  pw.Widget buildSummaryRow(String label, double value,
    ...
    return pw.Row(
      ...
      children: [
        ...
        pw.Text(
          ...
          style: pw.TextStyle(fontWeight: bold, color: color),
        ),
      ],
    );
  }
  ...
  pw.Widget buildWeeklyCard(Map<String, dynamic> data, bool isProfit) {
    ...
    return pw.Container(
      ...
      child: pw.Column(
        ...
        children: [
          pw.Text("Minggu ke-$mingguKe",
              style:
                  pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 10)),
          pw.SizedBox(height: 4),
          pw.Text(isProfit ? "Profit" : "GMV",
              style: const pw.TextStyle(fontSize: 9)),
          pw.SizedBox(height: 4),
          pw.Text(currencyFormat.format(amount),
              style: pw.TextStyle(
                  color: color, fontWeight: pw.FontWeight.bold, fontSize: 12)),
        ],
      ),
    );
  }
    ...
    pw.MultiPage(
      ...
      build: (context) {
        return [
          ...
          pw.Text("Dicetak oleh: $nama"),
          pw.Text("Dicetak pada: ${DateFormat('dd MMMM yyyy', 'id_ID').format(DateTime.now())}"),
              ...
              children: [
                ...
                buildSummaryRow(
                    ...
                    totalUnpaidSalary +
                        ...
                        totalOtherExpenses,
                    isHighlight: true),
                buildSummaryRow("Est. Keuntungan Bersih", netProfit,
                    isHighlight: true),
              ],
            ),
          ),
          ...
          pw.Wrap(
            ...
            runSpacing: 8,
            children:
                weeklySummary.map((data) => buildWeeklyCard(data, false)).toList(),
          ),

          pw.SizedBox(height: 12),

          // Weekly Profit
          pw.Text("Weekly Profit (5%)",
              style: pw.TextStyle(
                  fontSize: 16, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 8),
          pw.Wrap(
            ...
            runSpacing: 8,
            children:
                weeklySummary.map((data) => buildWeeklyCard(data, true)).toList(),
          ),

          pw.SizedBox(height: 16),

          pw.Text("Detail Cashflow",
              style: pw.TextStyle(
                  fontSize: 16, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 6),

          pw.Table.fromTextArray(
            headers: ["Tanggal", "Deskeipsi", "Kategori", "Tipe", "Nominal"],
            data: cashflowList.map((item) {
              final tanggal = formatTanggal(item['tanggal']);
              final deskripsi = item['deskripsi'] ?? item['kategori'];
              ...
              return [tanggal, deskripsi, kategori, tipe, nominal];
            }).toList(),
